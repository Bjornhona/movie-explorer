...
describe('useWishlistMovies', () => {
  ...
  it('should fetch and set wishlist movies on initial load', async () => {
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ results: mockMoviesPage1, page: 1, total_pages: 2, total_results: 3 }),
    });
    const { result } = renderHook(() => useWishlistMovies(accountId, sessionId));
    await waitFor(() => {
  ...
  it('should set error on fetch failure', async () => {
    ...
    const { result } = renderHook(() => useWishlistMovies(accountId, sessionId));
    await waitFor(() => {
      expect(result.current.error).toBe('Failed to fetch wishlist movies');
      expect(result.current.movies).toEqual([]);
    });
  });

  it('should append new unique wishlist movies and deduplicate on loadMore', async () => {
    // First page
    (fetch as any).mockResolvedValueOnce({
      ok: true,
      json: async () => ({ results: mockMoviesPage1, page: 1, total_pages: 2, total_results: 3 }),
